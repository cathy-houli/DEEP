    ...
    if (opt->input_mode==FA_FILE)
    {
        while((i<READ_BUF_LENGTH)&&fgets(f_line,MAX_STRING_LENGTH,file_1)!=NULL)
        {
            if(f_line[0] == '>')
            {
                ...
                {
                    if(f_line[0] == '>') sscanf(f_line,">%s",read[i].name);
                    else return 0;
                    if(fgets(f_line,MAX_STRING_LENGTH,file_2)!=NULL)
                    {
                        read[i].length = strlen(f_line)-1;
...

                    {
                        ...
                        for(x = 0;x< read[i].cand_num;x++)
                        {
                            ...
                            //+MD
                                ...
                                read[i+1].name,(read[i].cand[x].strand==0)?read[i].seq:read[i].rseq,(read[i].cand[x].strand==0)?read[i].qual:read[i].rqual,TAG);
                            }
                    ...
                    read[i+1].MapQ = generate_MapQ(read+i+1);
                    {
                        ...
                        for(x = 0;x< read[i+1].cand_num;x++)
                        {
                            ...
                            //+MD
                                ...
                                read[i].name,(read[i+1].cand[x].strand==0)?read[i+1].seq:read[i+1].rseq,(read[i+1].cand[x].strand==0)?read[i+1].qual:read[i+1].rqual,TAG);
                            }
