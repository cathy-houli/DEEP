...
{
	fprintf(stderr, "Load the BWT genome index files...");
	opt->idx->bwt = bwa_idx_load_bwt(hint);
	opt->idx->bns = bns_restore(hint);

	opt->idx->pac = (uint8_t*)calloc(opt->idx->bns->l_pac/4+1, 1);
	fseek(opt->idx->bns->fp_pac, 0, SEEK_SET);
	fread(opt->idx->pac, 1, opt->idx->bns->l_pac / 4 + 1, opt->idx->bns->fp_pac);

	int i = 0;
	unsigned int start = 0;
	for (i = 0; i < opt->idx->bns->n_seqs; i++)
	{
        ...
        opt->chr->list[i].c_hash = NULL;
        opt->chr->list[i].c_num = NULL;
...
int load_index(struct m_opt *opt)
{
    ...
    {
        fprintf(stderr, "[DEEP]Error! Cannot get reference index...\n");
        return 1;
    }
    return 0;
}
